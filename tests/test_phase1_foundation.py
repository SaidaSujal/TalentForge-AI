...
def client():
    ...
    os.environ.setdefault("DATABASE_URL", "postgresql+psycopg://test:test@localhost/testdb?sslmode=require")

    # Patch the DB health check so tests don't need a real database
    with patch("app.db.session.check_database_connection", new_callable=AsyncMock, return_value=True):
...
class TestHealthEndpoint:
    ...
    def test_health_status_unhealthy_when_db_down(self, client: TestClient):
        ...
        with patch("app.routes.health.check_database_connection", new_callable=AsyncMock, return_value=False):
            response = client.get("/api/v1/health")
        data = response.json()
...
class TestModelRouter:
    ...
    def test_jd_draft_routes_to_small(self):
        from app.services.ai.model_router import get_model_alias, ModelAlias
